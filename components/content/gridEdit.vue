<template>
  <div>
    ...
    <DsfrCheckbox v-model="isGridEnabled" label="Activer la vue en grille" name="grid-enabled" />
    {{ message }}
    <div class="grid-container fr-m-n2v">
      ...
      >
        <ContentGridItemEdit
          ...
          :key="index" v-model="contents[index]"
          :message="message"
          tag-name="li"
...
const gridColumnWidth = computed<number|undefined>(() => gridUnderlayRef?.value?.getBoundingClientRect().width / 6 || undefined) // TODO watch window resize
const message = ref("")
const targetColumnIndex = ref<number>(-1)
const initColumnIndex = ref<number>(-1)
const onGrid = computed<boolean>(() => targetColumnIndex.value < 6 && targetColumnIndex.value >= 0)

function columnIndexFromX(xPosition: number): number {
  if (!gridColumnWidth.value || !gridUnderlayRef.value) throw "Layout not setup yet"
  const distFromBegin = xPosition - gridUnderlayRef.value.getBoundingClientRect().x
...
function isInTheEnd(elementIndex: number): boolean {
  ...
  const distFromBegin = element.clientLeft + element.clientWidth - gridUnderlayRef.value.clientLeft - gridUnderlayRef.value.clientWidth
  return distFromBegin === 0
}

function applyOnUnderlayColumns(start: number, end: number, func: (colElement: Element) => void) {
  if (!gridUnderlayRef.value) throw "Layout not setup yet"
  let element
...
function onDragBegin(event: DragEventInit, index: number) {
  const xPosition = blockGridRef.value.children.item(index).getBoundingClientRect().x
  initColumnIndex.value = columnIndexFromX(xPosition)
}

function onDrag(event: DragEvent) {
  targetColumnIndex.value = columnIndexFromX(event.clientX)
  applyOnUnderlayColumns(initColumnIndex.value, targetColumnIndex.value, (colElement) => {
